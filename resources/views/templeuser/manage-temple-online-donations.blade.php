                                ...
                                <div class="profile-tab tab-menu-heading border-bottom-0">
                                    <nav class="nav main-nav-line p-0 tabs-menu profile-nav-line border-0 br-5 mb-0 full-width-tabs">
                                        <a class="nav-link mb-2 mt-2 " href="{{ route('templedonation.manage') }}"
                                            ...
                                            <a class="nav-link mb-2 mt-2 {{ Request::is('templeuser/manage-temple-online-donations') ? 'active' : '' }}" href="{{route('templedonation.manageonline')}}"
                                            onclick="changeColor(this)">Online Donations</a>

                                        <a class="nav-link mb-2 mt-2" href=""
                                        ...
                                        <table id="file-datatable" class="table table-bordered text-nowrap key-buttons border-bottom">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="border-bottom-0">Payment Number</th>
                                                    <th class="border-bottom-0">Action</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @foreach($onlinedonations as $index => $donation)
                                                <tr>
                                                    <td>{{ $index + 1 }}</td>
                                                    ...
                                                    <td>{{ $donation->payment_number }}</td>
                                                    <td>
                                                        <a href="{{ route('templedonation.editonline', $donation->id) }}" class="btn btn-warning btn-sm"><i class="fa fa-edit"></i></a>
                                                     
                                                        <form id="delete-form-{{ $donation->id }}" action="{{ route('templedonation.destroyonline', $donation->id) }}" method="POST" style="display:inline;">
                                                            @csrf
                                                            @method('DELETE')
