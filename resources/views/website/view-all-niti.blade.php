...
<div class="timeline">
    @foreach ($mergedNitiList as $index => $niti)
        @php
            ...
            $status = strtolower($niti['niti_status']);
            $side = $index % 2 === 0 ? 'left' : 'right';

            $statusClass = match ($status) {
                'completed' => 'Completed',
                'running', 'started' => 'GoingOn',
                'upcoming' => 'Upcoming',
                default => 'Upcoming'
            };
        ...
        <div class="timeline-item {{ $side }} {{ $statusClass }}">
            <div class="card timeline-content">
                <span class="badge {{ $statusClass }}">
                    <i class="fas {{ $icon }}"></i> {{ str_replace('GoingOn', 'Going On', $statusClass) }}
                </span>
                ...
                @if ($start || $end || $pause || $resume || $duration)
                    <div class="niti-times">
                        @if ($start)
                            <p><i class="fas fa-play-circle text-success"></i>
